...
# in this case is 58 cm
KNOWN_DISTANCE = 64.0
# initialize the known object width, which in this case, the mug is 8.6 cm wide
KNOWN_WIDTH = 9
# load the furst image that contains an object that is KNOWN TO BE 2 feet
# from our camera, then find the paper marker in the image, and initialize
# the focal length
WIDTH_MUG_PIXEL = 35
FOCALLENGTH = (WIDTH_MUG_PIXEL * KNOWN_DISTANCE) / KNOWN_WIDTH

def distance_to_camera(perWidth):
	# compute and return the distance from the maker to the camera
	return int((KNOWN_WIDTH * FOCALLENGTH) / perWidth)
    ...
    while True:
        if len(filaFrame) > 0:
            ...
            for i in range(count):
                ...
                # Apply NMS
                pred = non_max_suppression(pred, 0.3, 0.1, classes=opt.classes, agnostic=opt.agnostic_nms)
                ...
                for i, det in enumerate(pred):  # detections per image
                    ...
                    if len(det):
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            ...
                            print(width)
                            object_distance = distance_to_camera(width)
                            if(flagDistanceFilter):
                                ...
                                if(distancia < thresholdDistanceFilter):
                                    plot_one_box(xyxy, im0, label=str(object_distance), color=colors[1], line_thickness=2)
